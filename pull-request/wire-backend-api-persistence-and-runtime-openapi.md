...
This PR connects the diary backend API to concrete Drizzle/MySQL persistence and replaces the static OpenAPI YAML with a runtime `/openapi.json` endpoint generated from Hono route metadata.
...
- Added Drizzle configuration, MySQL schema, database client creation, repository implementations, and generated migration files.
...
- Updated backend package scripts and dependencies for Drizzle, MySQL, and OpenAPI support.

## What is not included

- This PR does not include frontend client regeneration from `/openapi.json`.
- This PR does not claim the migration was applied to a live MySQL database.
...
- API data is persisted through the Drizzle/MySQL repository implementations when using the production entrypoint.
...
- `bun run test` — passed, 91 tests
- `bun run typecheck` — passed
- `bun run lint` — passed
- `bun run db:generate` — passed

`bun run db:migrate` was attempted, but could not complete in this environment because the configured MySQL connection timed out with `connect ETIMEDOUT`.

## Notes

- `docs/v1/specification/db-specification.md` still references PostgreSQL, while the current backend implementation and dependencies use MySQL. The implementation follows the existing backend setup.
